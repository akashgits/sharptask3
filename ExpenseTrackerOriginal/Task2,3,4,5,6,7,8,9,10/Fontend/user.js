...
function premium(){
    axios.post('http://localhost:2121/ispremium',{token})
    .then(response=>{
...
const fromtobtn=document.getElementById('fromtobtn');
...
function PostToUser(e){
    ...
    axios.post(`http://localhost:2121/add-expense`,{token,expenseAmount,description,category}).
    then(response=>{
...
function loadPremiumUser(v){
    ...
    axios.post(`http://localhost:2121/getpremiumexpense?items=${v}`,{token})
    .then(response=>{
        console.log(response);
...
function createpagination(i){
    const pagination=document.getElementById('pagination');
    if(i===1){
        const p=`<a class="link Active" src="#" id="page(${i}) " onclick="page(${i})">${i}</a>`
        pagination.innerHTML=pagination.innerHTML+p;
    }
    else{
        const p=`<a class="link " src="#" id="page(${i}) "  onclick="page(${i})" >${i}</a>`
        pagination.innerHTML=pagination.innerHTML+p;
...
function page(i){
    var show=document.getElementById('ul_id');
    show.innerHTML="";
    axios.post(`http://localhost:2121/getpremiumexpense?page=${i}`,{token})
    .then(response=>{
        Main(response.data.respon);
...
function loadExpenseForUser(){
    ...
    axios.post('http://localhost:2121/getexpense',{token:token})
   .then(response=>{
    Main(response.data);
...
function deleteUser(id){
    axios.post(`http://localhost:2121/deleteexpense`,{id})
    .then(response=>{
        console.log(response)
        if(response.status==200){
 ...
 async function buypremium(e){
      ...
      //  e.preventDefault();
        const response  = await axios.post('http://localhost:2121/razorpay', {token} );
        console.log(response);
        var options =
        {
         ...
         "handler": function (response) {
             console.log(response);
             axios.post('http://localhost:2121/updatetransaction',{
...
//     axios.post("http://localhost:4444/get-expense",{UserId:UserId}).
//     then(response=>{
//         console.log(response)
...
//     axios.post("http://localhost:4444/delete-Expense",{id:id}).
//     then(res=>{
//         console.log(res)
//         removeFromScreen(id);
...
//     const response  = await axios.get('http://localhost:2121/purchase/premiummembership', { headers: {"Authorization" : token} });
//     console.log(response);
//     var options =
//     {
//      "key": response.data.key_id, // Enter the Key ID generated from the Dashboard
